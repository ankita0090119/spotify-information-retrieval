...
# ── CELL 5: TF-IDF Search with combined scoring ───────────────
...
# Filter the DataFrame for top genres
plot_df_filtered = sample_df[sample_df['track_genre'].isin(top_genres)]

# Determine the sample size, ensuring it's not larger than the population
sample_size = min(2000, len(plot_df_filtered))

# Sample from the filtered DataFrame
plot_df = plot_df_filtered.sample(sample_size, random_state=42)
